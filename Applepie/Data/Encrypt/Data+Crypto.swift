...
public extension Applepie where Base == Data {
    ...
    public func hashWithRSA2048Asn1Header(_ type: HashType, output: HashOutputType = .hex) -> String? {
        ...
        headerData.append(base)
        
        return encrypt(type, output: output)
    }
    ...
    public func encrypt(_ type: HashType, output: HashOutputType = .hex) -> String? {
        ...
        _ = digest.withUnsafeMutableBytes { (digestBytes: UnsafeMutablePointer<UInt8>) in
            base.withUnsafeBytes { (messageBytes: UnsafePointer<UInt8>) in
                let length = CC_LONG(base.count)
                switch type {
